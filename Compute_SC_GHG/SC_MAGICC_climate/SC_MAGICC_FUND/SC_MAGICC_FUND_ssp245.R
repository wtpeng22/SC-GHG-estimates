...
## CO2 concentration
MAGICC_data_C_ssp245_base = fread("./Module/Climate/MAGICC/outputs/ssp245/C_base.csv",encoding = "UTF-8",header = T);
MAGICC_data_C_ssp245_base = as.matrix(MAGICC_data_C_ssp245_base[V1 %in% MAGICC_row_target,-1]);

MAGICC_data_C_ssp245_CO2 = fread("./Module/Climate/MAGICC/outputs/ssp245/C_extra_CO2.csv",encoding = "UTF-8",header = T);
MAGICC_data_C_ssp245_CO2 = as.matrix(MAGICC_data_C_ssp245_CO2[V1 %in% MAGICC_row_target,-1]);

MAGICC_data_C_ssp245_CH4 = fread("./Module/Climate/MAGICC/outputs/ssp245/C_extra_CH4.csv",encoding = "UTF-8",header = T);
MAGICC_data_C_ssp245_CH4 = as.matrix(MAGICC_data_C_ssp245_CH4[V1 %in% MAGICC_row_target,-1]);

MAGICC_data_C_ssp245_N2O = fread("./Module/Climate/MAGICC/outputs/ssp245/C_extra_N2O.csv",encoding = "UTF-8",header = T);
MAGICC_data_C_ssp245_N2O = as.matrix(MAGICC_data_C_ssp245_N2O[V1 %in% MAGICC_row_target,-1]);

## Sea level rise (simulated by the Hector model)
Hector_row_target = year_target[-(1:50)]-2000;

MAGICC_data_S_ssp245_base = fread("./Module/Climate/Hector/outputs/ssp245/S_base.csv",encoding = "UTF-8",header = T);
MAGICC_data_S_ssp245_base = as.matrix(MAGICC_data_S_ssp245_base[V1 %in% Hector_row_target,-1]);

MAGICC_data_S_ssp245_CO2 = fread("./Module/Climate/Hector/outputs/ssp245/S_extra_CO2.csv",encoding = "UTF-8",header = T);
MAGICC_data_S_ssp245_CO2 = as.matrix(MAGICC_data_S_ssp245_CO2[V1 %in% Hector_row_target,-1]);

MAGICC_data_S_ssp245_CH4 = fread("./Module/Climate/Hector/outputs/ssp245/S_extra_CH4.csv",encoding = "UTF-8",header = T);
MAGICC_data_S_ssp245_CH4 = as.matrix(MAGICC_data_S_ssp245_CH4[V1 %in% Hector_row_target,-1]);

MAGICC_data_S_ssp245_N2O = fread("./Module/Climate/Hector/outputs/ssp245/S_extra_N2O.csv",encoding = "UTF-8",header = T);
MAGICC_data_S_ssp245_N2O = as.matrix(MAGICC_data_S_ssp245_N2O[V1 %in% Hector_row_target,-1]);
...
Monte_Carlo_T <-function(){
  ...
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,MAGICC_data_T_ssp245_base[,target_climate]);
    acco2 <<- c(acco2_his,MAGICC_data_C_ssp245_base[,target_climate]);
    sea <<- c(sea_his,MAGICC_data_S_ssp245_base[,target_climate]/100);
  }
  
  if(MarginalEmission_CO2 == TRUE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,MAGICC_data_T_ssp245_CO2[,target_climate]);
    acco2 <<- c(acco2_his,MAGICC_data_C_ssp245_CO2[,target_climate]);
    sea <<- c(sea_his,MAGICC_data_S_ssp245_CO2[,target_climate]/100);
  }
  
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == TRUE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,MAGICC_data_T_ssp245_CH4[,target_climate]);
    acco2 <<- c(acco2_his,MAGICC_data_C_ssp245_CH4[,target_climate]);
    sea <<- c(sea_his,MAGICC_data_S_ssp245_CH4[,target_climate]/100);
  }
  
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == TRUE){
    s_temp <<- c(s_temp_his,MAGICC_data_T_ssp245_N2O[,target_climate]);
    acco2 <<- c(acco2_his,MAGICC_data_C_ssp245_N2O[,target_climate]);
    sea <<- c(sea_his,MAGICC_data_S_ssp245_N2O[,target_climate]/100);
  }
}
...
write.csv(SC_CO2,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/SC_CO2_MAGICC_FUND.csv",quote = F,row.names = F)
write.csv(SC_CH4,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/SC_CH4_MAGICC_FUND.csv",quote = F,row.names = F)
write.csv(SC_N2O,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/SC_N2O_MAGICC_FUND.csv",quote = F,row.names = F)

write.csv(Global_damage_base,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/Global_damage_base_MAGICC_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_CO2,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/Global_damage_impulse_CO2_MAGICC_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_CH4,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/Global_damage_impulse_CH4_MAGICC_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_N2O,"Compute_SC_GHG/SC_MAGICC_climate/SC_MAGICC_FUND/ssp245/Global_damage_impulse_N2O_MAGICC_FUND.csv",quote = F,row.names = F)
