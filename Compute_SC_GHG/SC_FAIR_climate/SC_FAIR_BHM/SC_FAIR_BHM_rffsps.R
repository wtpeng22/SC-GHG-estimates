...
FAIR_data_T_rffsps_base = fread("./Module/Climate/FAIR/outputs/rffsps/T_base.csv",encoding = "UTF-8",header = T);
FAIR_data_T_rffsps_base = as.matrix(FAIR_data_T_rffsps_base[V1 %in% FAIR_row_target,-1]);

FAIR_data_T_rffsps_CO2 = fread("./Module/Climate/FAIR/outputs/rffsps/T_extra_CO2.csv",encoding = "UTF-8",header = T);
FAIR_data_T_rffsps_CO2 = as.matrix(FAIR_data_T_rffsps_CO2[V1 %in% FAIR_row_target,-1]);

FAIR_data_T_rffsps_CH4 = fread("./Module/Climate/FAIR/outputs/rffsps/T_extra_CH4.csv",encoding = "UTF-8",header = T);
FAIR_data_T_rffsps_CH4 = as.matrix(FAIR_data_T_rffsps_CH4[V1 %in% FAIR_row_target,-1]);

FAIR_data_T_rffsps_N2O = fread("./Module/Climate/FAIR/outputs/rffsps/T_extra_N2O.csv",encoding = "UTF-8",header = T);
FAIR_data_T_rffsps_N2O = as.matrix(FAIR_data_T_rffsps_N2O[V1 %in% FAIR_row_target,-1]);

Monte_Carlo_T <-function(){
  # 
  if(MarginalEmission_CO2 == TRUE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == FALSE){
    ssp_glob_temp_target <<- FAIR_data_T_rffsps_base[,target_climate];
    ssp_glob_temp_pulse_target <<- FAIR_data_T_rffsps_CO2[,target_climate];
  }
  
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == TRUE & MarginalEmission_N2O == FALSE){
    ssp_glob_temp_target <<- FAIR_data_T_rffsps_base[,target_climate];
    ssp_glob_temp_pulse_target <<- FAIR_data_T_rffsps_CH4[,target_climate];
  }
  
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == TRUE){
    ssp_glob_temp_target <<- FAIR_data_T_rffsps_base[,target_climate];
    ssp_glob_temp_pulse_target <<- FAIR_data_T_rffsps_N2O[,target_climate];
  }
...
write.csv(CSC_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/CSC_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/WSC_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(CSC_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/CSC_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/WSC_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(CSC_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/CSC_N2O_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/WSC_N2O_FAIR_BHM.csv",quote = F,row.names = F)

write.csv(Country_target_gdpcap_base,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/gdpcap_base_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_cc,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/gdpcap_cc_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/gdpcap_imp_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/gdpcap_imp_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/gdpcap_imp_N2O_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_base,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/Global_damage_base_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/Global_damage_imp_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/Global_damage_imp_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag5/Global_damage_imp_N2O_FAIR_BHM.csv",quote = F,row.names = F)
...
write.csv(CSC_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/CSC_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/WSC_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(CSC_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/CSC_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/WSC_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(CSC_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/CSC_N2O_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(WSC_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/WSC_N2O_FAIR_BHM.csv",quote = F,row.names = F)

write.csv(Country_target_gdpcap_base,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/gdpcap_base_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_cc,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/gdpcap_cc_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/gdpcap_imp_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/gdpcap_imp_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Country_target_gdpcap_imp_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/gdpcap_imp_N2O_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_base,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/Global_damage_base_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/Global_damage_imp_CO2_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/Global_damage_imp_CH4_FAIR_BHM.csv",quote = F,row.names = F)
write.csv(Global_damage_imp_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_BHM/rffsps/lag0/Global_damage_imp_N2O_FAIR_BHM.csv",quote = F,row.names = F)
