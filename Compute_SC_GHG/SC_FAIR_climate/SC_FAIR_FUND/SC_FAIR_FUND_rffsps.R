...
FAIR_row_target = year_target[-(1:50)]-1765;
...
## CO2 concentration
FAIR_data_C_rffsps_base = fread("./Module/Climate/FAIR/outputs/rffsps/C_base.csv",encoding = "UTF-8",header = T);
FAIR_data_C_rffsps_base = as.matrix(FAIR_data_C_rffsps_base[V1 %in% FAIR_row_target,-1]);

FAIR_data_C_rffsps_CO2 = fread("./Module/Climate/FAIR/outputs/rffsps/C_extra_CO2.csv",encoding = "UTF-8",header = T);
FAIR_data_C_rffsps_CO2 = as.matrix(FAIR_data_C_rffsps_CO2[V1 %in% FAIR_row_target,-1]);

FAIR_data_C_rffsps_CH4 = fread("./Module/Climate/FAIR/outputs/rffsps/C_extra_CH4.csv",encoding = "UTF-8",header = T);
FAIR_data_C_rffsps_CH4 = as.matrix(FAIR_data_C_rffsps_CH4[V1 %in% FAIR_row_target,-1]);

FAIR_data_C_rffsps_N2O = fread("./Module/Climate/FAIR/outputs/rffsps/C_extra_N2O.csv",encoding = "UTF-8",header = T);
FAIR_data_C_rffsps_N2O = as.matrix(FAIR_data_C_rffsps_N2O[V1 %in% FAIR_row_target,-1]);

## Sea level rise (simulated by the Hector model)
Hector_row_target = year_target[-(1:50)]-2000;

FAIR_data_S_rffsps_base = fread("./Module/Climate/Hector/outputs/rffsps/S_base.csv",encoding = "UTF-8",header = T);
FAIR_data_S_rffsps_base = as.matrix(FAIR_data_S_rffsps_base[V1 %in% Hector_row_target,-1]);

FAIR_data_S_rffsps_CO2 = fread("./Module/Climate/Hector/outputs/rffsps/S_extra_CO2.csv",encoding = "UTF-8",header = T);
FAIR_data_S_rffsps_CO2 = as.matrix(FAIR_data_S_rffsps_CO2[V1 %in% Hector_row_target,-1]);

FAIR_data_S_rffsps_CH4 = fread("./Module/Climate/Hector/outputs/rffsps/S_extra_CH4.csv",encoding = "UTF-8",header = T);
FAIR_data_S_rffsps_CH4 = as.matrix(FAIR_data_S_rffsps_CH4[V1 %in% Hector_row_target,-1]);

FAIR_data_S_rffsps_N2O = fread("./Module/Climate/Hector/outputs/rffsps/S_extra_N2O.csv",encoding = "UTF-8",header = T);
FAIR_data_S_rffsps_N2O = as.matrix(FAIR_data_S_rffsps_N2O[V1 %in% Hector_row_target,-1]);
...
Monte_Carlo_T <-function(){
  ...
  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,FAIR_data_T_rffsps_base[,target_climate]);
    acco2 <<- c(acco2_his,FAIR_data_C_rffsps_base[,target_climate]);
    sea <<- c(sea_his,FAIR_data_S_rffsps_base[,target_climate]/100);
  }

  if(MarginalEmission_CO2 == TRUE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,FAIR_data_T_rffsps_CO2[,target_climate]);
    acco2 <<- c(acco2_his,FAIR_data_C_rffsps_CO2[,target_climate]);
    sea <<- c(sea_his,FAIR_data_S_rffsps_CO2[,target_climate]/100);
  }

  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == TRUE & MarginalEmission_N2O == FALSE){
    s_temp <<- c(s_temp_his,FAIR_data_T_rffsps_CH4[,target_climate]);
    acco2 <<- c(acco2_his,FAIR_data_C_rffsps_CH4[,target_climate]);
    sea <<- c(sea_his,FAIR_data_S_rffsps_CH4[,target_climate]/100);
  }

  if(MarginalEmission_CO2 == FALSE & MarginalEmission_CH4 == FALSE & MarginalEmission_N2O == TRUE){
    s_temp <<- c(s_temp_his,FAIR_data_T_rffsps_N2O[,target_climate]);
    acco2 <<- c(acco2_his,FAIR_data_C_rffsps_N2O[,target_climate]);
    sea <<- c(sea_his,FAIR_data_S_rffsps_N2O[,target_climate]/100);
  }
}
...
pop_rffsps = fread("Module/Socioeconomic/outputs/FUND_data/rffsps_global_pop.csv",header = T)
gdp_rffsps = fread("Module/Socioeconomic/outputs/FUND_data/rffsps_global_gdp.csv",header = T)
pop_rffsps = as.matrix(pop_rffsps[V1 %in% dataupdate_year:end_year,-1])
gdp_rffsps = as.matrix(gdp_rffsps[V1 %in% dataupdate_year:end_year,-1])
...
diagnostic_FUND<-function(run_time){
  ...
  gdp_target <<- matrix(as.numeric(gdp_rffsps[,target_scenario]),nrow = (end_year-dataupdate_year+1))/10^9
  pop_target <<- matrix(as.numeric(pop_rffsps[,target_scenario]),nrow = (end_year-dataupdate_year+1))/10^6
...
#   print(run_time)
#   diagnostic_FUND(run_time)
#   SC_CO2[run_time,] = SC_FUND_CO2;
#   SC_CH4[run_time,] = SC_FUND_CH4*100;
#   SC_N2O[run_time,] = SC_FUND_N2O*1000;
#   Global_damage_base[run_time,] = rowSums(damage_base);
#   Global_damage_impulse_CO2[run_time,] = rowSums(damage_CO2imp);
#   Global_damage_impulse_CH4[run_time,] = rowSums(damage_CH4imp);
#   Global_damage_impulse_N2O[run_time,] = rowSums(damage_N2Oimp);
# }
...
write.csv(SC_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/SC_CO2_FAIR_FUND.csv",quote = F,row.names = F)
write.csv(SC_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/SC_CH4_FAIR_FUND.csv",quote = F,row.names = F)
write.csv(SC_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/SC_N2O_FAIR_FUND.csv",quote = F,row.names = F)

write.csv(Global_damage_base,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/Global_damage_base_FAIR_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_CO2,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/Global_damage_impulse_CO2_FAIR_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_CH4,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/Global_damage_impulse_CH4_FAIR_FUND.csv",quote = F,row.names = F)
write.csv(Global_damage_impulse_N2O,"Compute_SC_GHG/SC_FAIR_climate/SC_FAIR_FUND/rffsps/Global_damage_impulse_N2O_FAIR_FUND.csv",quote = F,row.names = F)
