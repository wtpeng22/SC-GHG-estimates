...
target_scenario = "rffsps"
# Load the rffsps emissions scenarios
library(dplyr)
library(data.table)
library(openxlsx)
library(arrow)
...
sample_order = read.csv("../../Sample/climate_scenario_sample.csv")
...
run_mc <- function(rn, # run number
  ...
  emissions_original <- as.matrix(read.csv(file.path(rundir,hector_csv_file),header = F))
  ...
  # for(i in 1574:1574) {
    ...
    emissions_sample <- sample_order$x[i]
    emissions <- emissions_original
    emissions[(2020-1765+5):(2300-1765+5),2] <- rffsp_co2_emissions[rffsp_co2_emissions$sample == emissions_sample,]$value - as.numeric(emissions[(2020-1765+4):(2300-1765+4),3])
    emissions[(2020-1765+5):(2300-1765+5),4] <- rffsp_ch4_emissions[rffsp_ch4_emissions$sample == emissions_sample,]$value
    emissions[(2020-1765+5):(2300-1765+5),5] <- rffsp_n2o_emissions[rffsp_n2o_emissions$sample == emissions_sample,]$value
    fq_emissions_name <- file.path(tmpdir, hector_csv_file)

    if(runname_mc == "rffsps_1"){
      emissions_baseline = emissions;
      write.table(emissions_baseline, fq_emissions_name,quote = F,row.names = F,col.names =F,sep=",")
    }
    if(runname_mc == "rffsps_2"){
