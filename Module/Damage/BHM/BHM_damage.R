...
Burke_damage <- function(temp, rid) { return(mb[rid + 1,1] * temp + mb[rid + 1,2] * temp^2) }
...
warming_effect <- function(temp, temp_baseline, gdpcap_tm1, rid, out_of_sample=T){
  ...
  return(Burke_damage(temp,rid) - Burke_damage(temp_baseline,rid))
}
