...
runwithoutdamage=TRUE;
#判断气候损失对人口的影响是否会转入下一年
runwithoutpopulationperturbation=TRUE;

model_choose="BestGuess";
a=list.files("Module/Damage/FUND/FUND_base_input");
datalist=lapply(a,function(name){
  read.table(paste("Module/Damage/FUND/FUND_base_input/",name,sep=""),sep=",");
});
...
PopulationComponent<-function(pgrowth,leave,enter,dead)#参数均为矩阵，来自不同的函数结果
{
  ...
  population[dataupdate_year_index,] <<- as.vector(as.matrix(pop_target[1,]));
  if(t>dataupdate_year_index)  {
    if(runwithoutpopulationperturbation==FALSE){
    population_factor <<-(enter[t-1,]/1000000)-(leave[t-1,]/1000000)-(ifelse(dead[t-1,]>=0,dead[t-1,]/1000000,0));
    }else{
      population_factor <<- 0;
    }
    population[t,]<<-as.vector(as.matrix(pop_target[t-dataupdate_year_index+1,]/pop_target[t-dataupdate_year_index,]))*(population[t-1,] + population_factor);
  }
...
SocialEconomicComponemt<-function(ypcgrowth,pgrowth,eloss,sloss,mitigationcost,Area,globalpopulation,population,population1){
  ...
  income[dataupdate_year_index,] <<- as.vector(as.matrix(gdp_target[1,]));
  if(t>dataupdate_year_index)  {
    if(t_value>=FromSimulationYear_value&&runwithoutdamage==FALSE){oldincome = income[t-1,] - consleak*eloss[t-1,]/10} else{
      oldincome = income[t-1,] - 0;
    }
    income[t,]<<-as.vector(as.matrix(gdp_target[t-dataupdate_year_index+1,]/gdp_target[t-dataupdate_year_index,]))*oldincome-mitigationcost[t-1,];
  }
  ...
  if(t>=dataupdate_year_index){
    urbpop[t,]<<-as.vector(as.matrix(urb_pop_ssp[t-dataupdate_year_index+1,-1]));
  }
...
ClimateRegionComponent<-function(s_temp_exo,acco2_exo,sea_exo){
  t=clock_Current(year_current);
  s_temp <<- s_temp_exo;
  acco2 <<- acco2_exo
  sea <<- sea_exo;
...
Playground_FUND_damage<-function(YearTorun){     # CALCULATE DAMAGE
  for(t in 1:YearTorun){
    year_current<<-t;
    
    if(t==1){
      ClimateRegionComponent(s_temp,acco2,sea)
