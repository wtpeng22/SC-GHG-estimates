...
# EIND<-t(DICE_data[4,2:(Time_periods+1)]);
# etree=t(DICE_data[1,2:(Time_periods+1)])*44/12;
# forcoth=t(DICE_data[5,2:(Time_periods+1)]);
...
YGROSS_base=t(DICE_data[3,2:(Time_periods+1)]);
l = t(DICE_data[2,2:(Time_periods+1)]);
...
## update TATM based on various climate models
  TATM[1:(Time_periods-1)] <<- DICE_temp
