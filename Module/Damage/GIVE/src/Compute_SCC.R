...
####Run the GIVE model under the probability mode
trials_num = 4
SCC_withslr_sum = matrix(nrow = trials_num,ncol = 1)
SCC_noslr_sum = matrix(nrow = trials_num,ncol = 1)

system.time({
  for (i in 1:trials_num) {
    print(i)
    SCC_withslr_sum[i] = diagnostic_GIVE_model_MonteCarlo()
  }
});

mean(SCC_withslr_sum)
