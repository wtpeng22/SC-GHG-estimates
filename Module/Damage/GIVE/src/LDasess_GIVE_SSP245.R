...
socioeconomics_source = "SSP";   ###RFF和SSP二选一
SSP_scenario = "SSP245";
RFFSPsample = NULL;
...
#####Run climate model
sample_fair_idx = 0;
Hector_ssp245_temp = read.csv("../../Commonscenario/Hector/output/Hector_ssp245_temp.csv")
Run_climate_model <- function(Marginal_choice){
  ...
  temperature_base <<- Hector_ssp245_temp$value;
  temperature_allyears <<- Hector_ssp245_temp$value
...
write.csv(global_total_damage_base,"./output/Global_total_damage_determine.csv",quote = F,row.names = F)

Global_damage_sector_determine = cbind(rowSums(energy_costs_dollar),rowSums(agcost),rowSums(mortality_costs)/10^9,global_slr_damage_base)
write.csv(Global_damage_sector_determine,"./output/Global_damage_sector_determine.csv",quote = F,row.names = F)

energy_costs_dollar_fund = matrix(0,nrow = length(timestep_damage),ncol = length(fund_regions))
mortality_costs_fund = matrix(0,nrow = length(timestep_damage),ncol = length(fund_regions))
agcost_fund = agcost;
slr_costs_fund = matrix(0,nrow = length(timestep_damage),ncol = length(fund_regions))
slr_fund_idx = input_output_mapping_int[match(ciam_countries,countries)]

for (i in 1:length(ag_mapping_input_regions)) {
  for (t in 1:length(timestep_damage)) {
    energy_costs_dollar_fund[t, input_output_mapping_int[i]] <- energy_costs_dollar_fund[t, input_output_mapping_int[i]] + energy_costs_dollar[t, i]
    mortality_costs_fund[t, input_output_mapping_int[i]] <- mortality_costs_fund[t, input_output_mapping_int[i]] + mortality_costs[t, i]
  }
}

for (i in 1:length(slr_fund_idx)) {
  for (t in 1:length(timestep_damage)) {
     slr_costs_fund[t, slr_fund_idx[i]] <- slr_costs_fund[t, slr_fund_idx[i]] + OptimalCost_country[t, i]*pricelevel_2010_to_2005
  }
}

Regional_damage_determine = energy_costs_dollar_fund+mortality_costs_fund/10^9+slr_costs_fund+agcost_fund
write.csv(Regional_damage_determine,"./output/Regional_damage_determine.csv",quote = F,row.names = F)

# write.csv(cbind(countries,input_output_mapping,input_output_mapping_int),"./output/countries_FUND_index.csv",quote = F,row.names = F)

####Test the GIVE model under the probability mode
trials_num = 10
Global_damage_sector_MC = matrix(nrow = length(damages_first:model_last),ncol = trials_num)

system.time({
  for (i in 1:trials_num) {
    print(i)
    diagnostic_GIVE_model_MonteCarlo()
    Global_damage_sector_MC[,i] = global_sector_damage_base
  }
});
...
Global_damage_ciam_MCPara = matrix(nrow = length(damages_first:model_last),ncol = 1)
Countrial_damage_ciam_MCPara = matrix(nrow = length(damages_first:model_last),ncol = length(segments))
Global_damage_GDP_MCPara = matrix(nrow = length(damages_first:model_last),ncol = 1)
diagnostic_GIVE_model_deterministic()

####The damages are measured by the $2005
Global_damage_ciam_MCPara = global_slr_damage_base
Countrial_damage_ciam_MCPara = rowSums(OptimalCost_country)*pricelevel_2010_to_2005
####The GDP is measured by the $2005 
Global_GDP_MCPara = gdp_global*pricelevel_2011_to_2005

####For the Energy, Agriculture, and Mortality data, we do the Monte Carlo runs
trials_num = 10000
Global_Energydamage_MCPara = matrix(nrow = length(damages_first:model_last),ncol = trials_num)
Global_Agdamage_MCPara = matrix(nrow = length(damages_first:model_last),ncol = trials_num)
Global_Mortalitydamage_MCPara = matrix(nrow = length(damages_first:model_last),ncol = trials_num)
Global_damage_sector_MCPara = matrix(nrow = length(damages_first:model_last),ncol = trials_num)
...
for (run_time in 1:trials_num) {
  Global_Energydamage_MCPara[,run_time] = res1.p[[run_time]][[1]];
  Global_Agdamage_MCPara[,run_time] = res1.p[[run_time]][[2]];
  Global_Mortalitydamage_MCPara[,run_time] = res1.p[[run_time]][[3]];
  Global_damage_sector_MCPara[,run_time] = res1.p[[run_time]][[4]];
}

Global_damage_sector_MCPara_mean = cbind(rowMeans(Global_Energydamage_MCPara),
  rowMeans(Global_Agdamage_MCPara),
  rowMeans(Global_Mortalitydamage_MCPara));

write.csv(Global_Energydamage_MCPara,"./output/Global_Energydamage_MCPara.csv",quote = F,row.names = F)
write.csv(Global_Agdamage_MCPara,"./output/Global_Agdamage_MCPara.csv",quote = F,row.names = F)
write.csv(Global_Mortalitydamage_MCPara,"./output/Global_Mortalitydamage_MCPara.csv",quote = F,row.names = F)
write.csv(Global_damage_sector_MCPara,"./output/Global_damage_sector_MCPara.csv",quote = F,row.names = F)
write.csv(Global_damage_sector_MCPara_mean,"./output/Global_damage_sector_MCPara_mean.csv",quote = F,row.names = F)

write.csv(Global_damage_ciam_MCPara,"./output/Global_damage_ciam_MCPara.csv",quote = F,row.names = F)
write.csv(Global_GDP_MCPara,"./output/Global_GDP_MCPara.csv",quote = F,row.names = F)

# plot(2020:2300,(global_total_damage_perturb - global_total_damage_base)*(1/1.02^(1:281-1))*12/44)
# plot(2020:2300,(global_sector_damage_base/rowSums(gdp)))
